    ...
    if (instruction->opcode == I_INCBIN) {
        ...
        if (fp == NULL)
            ...
            while (t--) {
                ...
                while (l > 0) {
                    long m =
                        fread(buf, 1, (l > sizeof(buf) ? sizeof(buf) : l), fp);
                    if (!m) {
                        /*
                         ...
                         * it.
                         */
                        error(ERR_NONFATAL, "`incbin': unexpected EOF while" " reading file `%s'", fname);
                        t = 0;  /* Try to exit cleanly */
                        break;
                    }
                    out(offset, segment, buf, OUT_RAWDATA + m, NO_SEG, NO_SEG);
                    l -= m;
                }
            }
